...
|__features
   ...
   |__environment.py 					# This is the entry point  
   										  and the place to define code to run before and after certain events during your testing
   										  (before_all, after_all, before_feature, after_feature, before_step, after_step, etc...)
...
|__pages
   |__base_page.py 						# Define some basic functions to interacte with web page 
   										  (e.g. get html element, load website, click action, send key to text field, etc...)
   |__lead_capture_form.py 				# Define all HTML elements and function which need to perform a test on Lead Capture form
   										  (e.g. submit form, submit empty form, verify input data, verify total records, etc...)
